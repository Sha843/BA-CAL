...
import time
import math
# from train import clip_gradient
# from clip_grad import clip_gradient
...
def train_one_batch_contrastive(batch, model, optimizer, gradient_norm_queue, prototype_queue, args):
    ...
    # y_pred,_ = model.clf(representation)
    ...
    #     print(f"Parameter: {name}, Sparse: {param.is_sparse}")
...
def clip_gradient(model, gradient_norm_queue, args):
    if args.gradient_clip_value is not None:
        max_norm = max(gradient_norm_queue) if gradient_norm_queue else 1.0
        ...
        if total_norm > max_norm * args.gradient_clip_value:
            print(f'Clipping gradients with total norm {total_norm.item()} and max norm {max_norm}')
